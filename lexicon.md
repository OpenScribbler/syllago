...
| **Provider Source Manifest** | A per-provider YAML declaring documentation URLs, fetch tier, change detection method, and last-verified baseline. Owned by capmon since the extraction; lives only in the capmon repo. Syllago holds no copy — it consumes capmon's output via the Capability Feed. | source manifest, provider manifest |
...
| Term | Definition | Aliases to avoid |
| --- | --- | --- |
| **Capmon** | The external OpenScribbler/capmon project that monitors provider documentation and publishes the **Capability Feed**; syllago is a consumer only. (Formerly syllago's in-repo four-stage pipeline, extracted 2026-07.) | capability monitor, cap-mon |
