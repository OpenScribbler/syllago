# Syllago Phase 1 + 2 — Implementation Plan

**Goal:** Expand syllago from a content manager to a codebase scanning, surprise detection, and agent config parity tool — covering Phase 1 (Provider Parity) and Phase 2 (Scan + Surprise Detection).
...
**Design Doc:** `docs/syllago-design-decisions.md`
...
5. **Verify integration** — `go build ./cmd/syllago && go test ./...`
...
**Covers:** Decision #1 (Tool Identity), #2 (Data Model), #3 (Provider Abstraction), #4 (ContextDocument Shape), #6 (.syllago/ Directory), #13 (Project Structure)
...
- Modify: `cli/cmd/syllago/main.go` (restructure around cobra)
- Create: `cli/cmd/syllago/main_test.go`

**Depends on:** Nothing

**Success Criteria:**
- [ ] `syllago` (no args) launches the existing TUI
- [ ] `syllago --help` shows available subcommands
- [ ] `syllago backfill` still works (migrated from hand-rolled check)
- [ ] `syllago version` prints version string
...
func TestRootCommandHelp(t *testing.T) {
	...
	out := buf.String()
	if !strings.Contains(out, "syllago") {
		t.Error("help output should contain 'syllago'")
	}
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go get github.com/spf13/cobra@latest
```

#### Step 3: Create root cobra command

Replace the current `main()` function in `cli/cmd/syllago/main.go`. The key change: `syllago` with no args runs the TUI (as `RunE` on the root command), subcommands are registered separately.
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./cmd/syllago/... && go build ./cmd/syllago && go test ./...
```

Expected: All tests pass. `./syllago` launches TUI. `./syllago version` prints version. `./syllago --help` shows command list.
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/model/...
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/provider/... && go test ./...
```
...
### Task 1.4: Create .syllago/ config package
...
- [ ] Load/Save to `.syllago/config.json`
- [ ] Returns zero-value config if file doesn't exist (first run)
- [ ] Init function creates `.syllago/` directory
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/config/...
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/output/...
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/parse/...
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/parity/...
```
...
### Task 2.4: Add `syllago init` command

**Files:**
- Create: `cli/cmd/syllago/init.go`
- Create: `cli/cmd/syllago/init_test.go`

**Depends on:** Task 1.1 (Cobra), Task 1.3 (Provider.Detect), Task 1.4 (config package)

**Success Criteria:**
- [ ] `syllago init` detects providers, shows findings, creates `.syllago/config.json`
- [ ] `--yes` skips interactive confirmation
- [ ] `--json` outputs structured JSON instead of human-readable
- [ ] If `.syllago/config.json` already exists, warns and exits (unless `--force`)
- [ ] Respects `SYLLAGO_NO_PROMPT=1` as equivalent to `--yes`
...
func runInit(cmd *cobra.Command, args []string) error {
	...
	// Check for --yes or SYLLAGO_NO_PROMPT
	yes, _ := cmd.Flags().GetBool("yes")
	if !yes && os.Getenv("SYLLAGO_NO_PROMPT") != "1" && !output.JSON {
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./cmd/syllago/... && go build ./cmd/syllago
```

Expected: Tests pass. `./syllago init --help` shows flags.

---

### Task 2.5: Add `syllago import` and `syllago parity` commands

**Files:**
- Create: `cli/cmd/syllago/import.go`
- Create: `cli/cmd/syllago/import_test.go`
- Create: `cli/cmd/syllago/parity.go`
- Create: `cli/cmd/syllago/parity_test.go`

**Depends on:** Task 1.1 (Cobra), Task 2.1-2.2 (parse package), Task 2.3 (parity package)

**Success Criteria:**
- [ ] `syllago import --from cursor` discovers and parses Cursor content
...
- [ ] `syllago parity` shows coverage matrix across detected providers
- [ ] `syllago parity --json` outputs structured Report
...
import (
	...
	"github.com/OpenScribbler/syllago/cli/internal/catalog"
	"github.com/OpenScribbler/syllago/cli/internal/output"
	"github.com/OpenScribbler/syllago/cli/internal/parity"
	"github.com/OpenScribbler/syllago/cli/internal/provider"
	"github.com/spf13/cobra"
)
...
```go
// cli/cmd/syllago/import_test.go
...
```go
// cli/cmd/syllago/parity_test.go
...
### Task 2.6: Add `syllago config` and `syllago info` commands

**Files:**
- Create: `cli/cmd/syllago/config_cmd.go`
- Create: `cli/cmd/syllago/config_cmd_test.go`
- Create: `cli/cmd/syllago/info.go`
- Create: `cli/cmd/syllago/info_test.go`

**Depends on:** Task 1.1 (Cobra), Task 1.4 (config package)

**Success Criteria:**
- [ ] `syllago config list` shows current provider selection
- [ ] `syllago config add <provider>` adds a provider
- [ ] `syllago config remove <provider>` removes a provider
- [ ] `syllago info` outputs machine-readable capability manifest
- [ ] `syllago info formats` lists supported output formats
- [ ] `syllago info providers` lists all known providers
...
var configListCmd = &cobra.Command{
	...
	RunE: func(cmd *cobra.Command, args []string) error {
		...
		if output.JSON {
			...
			if len(cfg.Providers) == 0 {
				fmt.Println("No providers configured. Run `syllago init` to set up.")
			} else {
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./cmd/syllago/... && go build ./cmd/syllago && ./syllago --help
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/scan/...
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/scan/detectors/...
```
...
- [ ] Each section wrapped in `<!-- syllago:auto:<category> -->` boundary markers
...
import "github.com/OpenScribbler/syllago/cli/internal/model"
...
func (e ClaudeEmitter) Emit(doc model.ContextDocument) (string, error) {
	...
	b.WriteString(fmt.Sprintf("*Generated by syllago for %s*\n\n", doc.ProjectName))
	...
	for _, section := range doc.Sections {
		...
		b.WriteString(fmt.Sprintf("<!-- syllago:auto:%s -->\n", cat))
...
func TestClaudeEmitterBasic(t *testing.T) {
	...
	if !strings.Contains(output, "<!-- syllago:auto:tech-stack -->") {
...
func TestClaudeEmitterSurprise(t *testing.T) {
	...
	if !strings.Contains(output, "<!-- syllago:auto:surprise -->") {
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/emit/...
```
...
**Success Criteria:**
- [ ] Cursor emitter produces `.mdc` format with YAML frontmatter and `# syllago:auto:*` markers
...
func (e CursorEmitter) Emit(doc model.ContextDocument) (string, error) {
	...
	b.WriteString("---\n")
	b.WriteString("description: Project context generated by syllago\n")
...
- [ ] Reconciler replaces `syllago:auto:*` sections with new emitter output
- [ ] Reconciler preserves `syllago:human:*` sections verbatim
- [ ] Unmarked content is preserved (graceful with pre-syllago files)
...
const (
	FormatHTML MarkerFormat = "html" // <!-- syllago:auto:name -->
	FormatYAML MarkerFormat = "yaml" // # syllago:auto:name
)
...
var (
	htmlOpenRe  = regexp.MustCompile(`<!--\s*syllago:(auto|human):(\S+)\s*-->`)
	htmlCloseRe = regexp.MustCompile(`<!--\s*/syllago:(auto|human):(\S+)\s*-->`)
	yamlOpenRe  = regexp.MustCompile(`#\s*syllago:(auto|human):(\S+)`)
	yamlCloseRe = regexp.MustCompile(`#\s*/syllago:(auto|human):(\S+)`)
)
...
func writeSection(b *strings.Builder, s Section, format MarkerFormat) {
	if format == FormatYAML {
		b.WriteString(fmt.Sprintf("# syllago:%s\n", s.Name))
...
<!-- syllago:auto:tech-stack -->
## Tech Stack
- Go 1.22
<!-- /syllago:auto:tech-stack -->

<!-- syllago:human:architecture -->
## Architecture
...
func TestReconcileReplacesAuto(t *testing.T) {
	existing := `<!-- syllago:auto:tech-stack -->
## Tech Stack
- Go 1.21
<!-- /syllago:auto:tech-stack -->
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/reconcile/...
```
...
import "github.com/OpenScribbler/syllago/cli/internal/scan"
...
**Codebase context:** Phase 1 commands (init, import, parity, config, info) are registered from Group 2. Phase 2 adds scan, drift, and baseline. The scanner (Group 3) and detectors (Group 4) provide the execution engine. Emitters and reconciler handle output. The drift engine compares against `.syllago/baseline.json`.
...
**Depends on:** Task 1.2 (model types), Task 1.4 (config package for .syllago/ path)

**Success Criteria:**
- [ ] `SaveBaseline()` serializes ContextDocument to `.syllago/baseline.json` with section hashes
...
func SaveBaseline(syllagoDir string, doc model.ContextDocument) error {
	...
	path := filepath.Join(syllagoDir, BaselineFileName)
	return os.WriteFile(path, data, 0644)
}

// LoadBaseline reads a baseline snapshot from disk.
func LoadBaseline(syllagoDir string) (*Baseline, error) {
	path := filepath.Join(syllagoDir, BaselineFileName)
	data, err := os.ReadFile(path)
	if err != nil {
...
func BaselineExists(syllagoDir string) bool {
	_, err := os.Stat(filepath.Join(syllagoDir, BaselineFileName))
	return err == nil
}
```
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./internal/drift/...
```
...
### Task 5.2: Add `syllago scan` command

**Files:**
- Create: `cli/cmd/syllago/scan.go`
- Create: `cli/cmd/syllago/scan_test.go`
...
**Success Criteria:**
- [ ] `syllago scan` runs all detectors, emits to configured providers, updates baseline
...
import (
	...
	"github.com/OpenScribbler/syllago/cli/internal/config"
	"github.com/OpenScribbler/syllago/cli/internal/drift"
	"github.com/OpenScribbler/syllago/cli/internal/output"
)
...
func TestScanDryRunDoesNotWrite(t *testing.T) {
	...
	syllagoDir := filepath.Join(tmp, ".syllago")
	if drift.BaselineExists(syllagoDir) {
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go test ./cmd/syllago/...
```
...
import (
	...
	"github.com/OpenScribbler/syllago/cli/internal/config"
	"github.com/OpenScribbler/syllago/cli/internal/drift"
	"github.com/OpenScribbler/syllago/cli/internal/emit"
	"github.com/OpenScribbler/syllago/cli/internal/output"
	"github.com/OpenScribbler/syllago/cli/internal/parity"
	"github.com/OpenScribbler/syllago/cli/internal/provider"
	"github.com/OpenScribbler/syllago/cli/internal/reconcile"
	"github.com/OpenScribbler/syllago/cli/internal/scan"
	"github.com/OpenScribbler/syllago/cli/internal/scan/detectors"
	"github.com/spf13/cobra"
)
...
func runScan(cmd *cobra.Command, args []string) error {
	...
	if !config.Exists(root) {
		yes, _ := cmd.Flags().GetBool("yes")
		if !yes && os.Getenv("SYLLAGO_NO_PROMPT") != "1" && !output.JSON {
			fmt.Println("No .syllago/config.json found. Run `syllago init` first, or use --yes to auto-detect.")
	...
	// Update baseline
	if !dryRun {
		syllagoDir := config.DirPath(root)
		if err := drift.SaveBaseline(syllagoDir, result.Document); err != nil {
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go build ./cmd/syllago && ./syllago scan --help
```

---

### Task 5.3: Add `syllago drift` command

**Files:**
- Create: `cli/cmd/syllago/drift.go`
- Create: `cli/cmd/syllago/drift_test.go`

**Depends on:** Task 1.1 (Cobra), Task 3.1 (scanner), Task 4.7 (detector registry), Task 5.1 (drift package)

**Success Criteria:**
- [ ] `syllago drift` compares current scan against baseline, reports changes
...
func TestDriftCommandClean(t *testing.T) {
	...
	drift.SaveBaseline(syllagoDir, doc)
...
func runDrift(cmd *cobra.Command, args []string) error {
	...
	syllagoDir := config.DirPath(root)
	if !drift.BaselineExists(syllagoDir) {
		output.PrintError(1, "no baseline found", "Run `syllago scan` first to create a baseline")
		return fmt.Errorf("no baseline")
	}

	baseline, err := drift.LoadBaseline(syllagoDir)
	if err != nil {
		return err
	}
...
### Task 5.4: Add `syllago baseline` command

**Files:**
- Create: `cli/cmd/syllago/baseline.go`
- Create: `cli/cmd/syllago/baseline_test.go`

**Depends on:** Task 1.1 (Cobra), Task 5.1 (drift package), Task 5.2 (scan for baseline creation)

**Success Criteria:**
- [ ] `syllago baseline` accepts current codebase state without regenerating files
...
```go
// cli/cmd/syllago/baseline_test.go
...
func runBaseline(cmd *cobra.Command, args []string) error {
	...
	if output.JSON {
		...
		fmt.Printf("  Path: %s/%s\n", syllagoDir, drift.BaselineFileName)
		fmt.Println("  Future `syllago drift` will compare against this baseline.")
	}
...
```bash
cd /home/hhewett/.local/src/syllago/cli && go build ./cmd/syllago && ./syllago --help
```
