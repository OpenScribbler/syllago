# TUI Redesign: Sidebar Layout + Mouse Support + Modals + Syllago Palette
...
The current Syllago TUI uses full-screen replacement navigation: selecting a category replaces the entire screen with the items list, selecting an item replaces it with the detail view, and pressing Esc goes back. This creates several issues:
...
4. **Syllago color palette** — brand-aligned mint green + lavender purple with all colors passing WCAG AA
...
| Color scheme | Syllago palette (mint + viola) | Brand-aligned, all colors pass WCAG AA on both light and dark terminals |
...
8. **Brand-aligned** — Syllago mint green + viola purple palette throughout
