...
7. **Update flow** (D17): explicit-decision routing per D16 state, Replace-in-place per D20 byte contract as default for `Clean`, Drop stale record as default for `Modified/edited` and `Modified/missing`, Keep as default for `Modified/unreadable` per D17's reason-varying default table. CLI flags `--on-clean` and `--on-modified`; TUI `installUpdateModal` / `installModifiedModal` as the interactive surface. Depends on Phase 6's `verification_state` function.
