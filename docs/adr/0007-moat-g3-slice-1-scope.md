...
## Addendum 2: `syllago moat sign` — Publisher-Side Bundle Assembly

**Date:** 2026-04-22 · **Bead:** `syllago-92i4c` · **Extends:** D2 (publishing workflow)

### Context

Slice 1 defined the consumer path (`VerifyManifest`) but left the publisher path implicit: *"Production publishers (the MOAT Publisher Action) emit a `.sigstore` bundle alongside each artifact."* Bead `syllago-92i4c` (Phases 2 and 3) adds the CLI half of that workflow:

1. **Phase 2 — Online mode:** Assembles a verified sigstore v0.3 bundle from a raw Rekor API response captured by a GitHub Actions Publisher Action. The Rekor entry must have signed `sha256(manifest.json)`. Bundle is round-trip verified with `VerifyManifest` before being written. Entry point: `BuildBundle(rekorRaw, manifestBytes)` in `cli/internal/moat/bundle_builder.go`.

2. **Phase 3 — Dev offline mode:** Produces a matched `(signature.bundle, trusted_root.json)` pair using sigstore-go's `VirtualSigstore` — a full offline CA that emits real sigstore bundle structures without any network calls. Entry point: `SignManifestDev(manifestBytes)` in `cli/internal/moat/dev_signer.go`.

Both modes are exposed via `syllago moat sign`:

```
syllago moat sign \
  --manifest ./registry/manifest.json \
  --rekor-raw ./ci-artifacts/rekor-response.json   # online

syllago moat sign \
  --manifest ./test-registry/manifest.json \
  --dev-trusted-root ./devroot                     # dev offline
```

### Decision: VirtualSigstore as the smoke-fixture CA

`SignManifestDev` uses `VirtualSigstore` because it:
- Produces real sigstore bundle structures (not mocks) that round-trip through `VerifyManifest` unchanged.
- Requires no external services or credentials.
- Is already shipped in `sigstore-go`'s testing package.

The output is explicitly marked dev-only: the CA keys are ephemeral, the root is not anchored in any public TUF repository, and the dev issuer (`https://dev.syllago.local/oidc`) is an unambiguous placeholder.

The dev bundle passes `VerifyManifest` with `verify.WithIntegratedTimestamps(1)` — it carries a real sigstore inclusion proof (generated via `VirtualSigstore.GetInclusionProof`), not merely an inclusion promise.

### VirtualSigstore patching

`VirtualSigstore.Sign` uses a deprecated `tlog.NewEntry` path that leaves three fields nil. `entityToBundleJSON` patches them before building the v0.3 bundle:

1. **KindVersion** — left nil by `NewEntry`; required by `ParseTransparencyLogEntry`. Patched to `{Kind: "hashedrekord", Version: "0.0.1"}`.

2. **InclusionPromise (SET)** — the SET lives in a private field on `tlog.Entry` inaccessible outside the package. `entry.Signature()` returns the *artifact* ECDSA signature, NOT the SET. Regenerated by reconstructing the canonical `RekorPayload` (`{Body, IntegratedTime, LogIndex, LogID}`) and calling `vs.RekorSignPayload`.

3. **InclusionProof** — absent from `NewEntry`. Generated via `vs.GetInclusionProof(tle.CanonicalizedBody)`. The proof's `LogIndex` (tree-local position, always 0 in a single-entry tree) is used — NOT `entry.LogIndex()` (VirtualSigstore's hardcoded global index 1000), which exceeds the single-entry tree size and causes an inclusion-proof validation error.

Additionally, `VirtualSigstore.RekorLogs()` and `CTLogs()` set `ID: []byte(hexString)` (ASCII hex as bytes). After JSON round-trip through `root.TrustedRoot`, `hex.EncodeToString(GetKeyId())` double-encodes the ID, breaking `VerifySET` key lookup. Fix: `hex.DecodeString(string(tl.ID))` before constructing the trusted root.

### Smoke fixture workflow

`tests/smoke/moat-trust-surfacing.sh` demonstrates the full end-to-end path using only CLI commands:

```bash
syllago moat sign \
  --manifest ./registry/manifest.json \
  --dev-trusted-root ./devroot
git -C ./registry add manifest.json.sigstore && git commit -m "add MOAT bundle"
syllago registry add "file:///path/to/registry" \
  --name smoke-moat \
  --signing-issuer "https://dev.syllago.local/oidc" \
  --signing-identity "https://dev.syllago.local/workflow/smoke@refs/heads/main"
syllago add --all --from smoke-moat --trusted-root ./devroot/trusted_root.json
```

Negative: `syllago add --from` with a wrong signing identity stored in config fails at verification time with `MOAT_IDENTITY_MISMATCH`, not at `registry add` time.

### What is NOT in scope

- Production use: dev bundles and dev trusted roots are not suitable for production registries. The dev signing identity is a placeholder; the CA keys are ephemeral and unanchored.
- GitHub Actions Publisher workflow: the CI side remains outside the syllago repo. The `--rekor-raw` flag provides the integration point for teams using the Publisher Action.
- Key rotation for `VirtualSigstore`: each `SignManifestDev` call creates a fresh ephemeral CA. Callers must use the matching `trusted_root.json` from the same call.
