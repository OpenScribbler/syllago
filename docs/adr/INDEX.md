...
| [0008](0008-capmon-synthetic-fixtures-correct.md) | Synthetic Fixtures Are Correct for capmon Extractor Tests | accepted | advisory | `cli/internal/capmon/extract_*/*`, `cli/internal/capmon/testdata/*` | two-layer test architecture; synthetic fixtures for mechanics, real-world landmarks at recognize layer; rejects audit P2.5 prescription |
| [0009](0009-per-provider-anchor-vs-per-provider-contenttype-anchor.md) | Per-Provider Anchor for capmon Change Issues | accepted | advisory | `cli/internal/capmon/report.go`, `cli/internal/capmon/check.go` | one `<!-- capmon-check: <slug> -->` anchor per provider; additive new functions; legacy per-(provider, contentType) path unchanged |
| [0010](0010-silent-skip-vs-comment-on-redetection.md) | Silent Skip on capmon Re-Detection | accepted | advisory | `cli/internal/capmon/check.go`, `cli/internal/capmon/report.go` | open issue is the signal; no comment appended on re-detection of the same provider's hash change |
| [0011](0011-collect-cache-hit-counts-via-providerstatus-counter-vs-per-e.md) | Cache-Hit Counter in ProviderStatus | accepted | advisory | `cli/internal/capmon/types.go`, `cli/internal/capmon/pipeline.go` | `SourcesCacheHit` field on ProviderStatus; Stage 1 loop increments it; capmon fetch reads it for --verbose output |
| [0012](0012-provider-validation-against-loadallsourcemanifests-vs-provid.md) | capmon fetch Provider Validation Against Source Manifests | accepted | advisory | `cli/cmd/syllago/capmon_cmd.go`, `cli/internal/capmon/sourceman.go` | validate --provider against LoadAllSourceManifests slugs, not AllProviders, to avoid silent no-op on providers without manifests |
