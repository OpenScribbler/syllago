...
Last updated: 2026-04-09. Authoritative sources: `cli/internal/provider/*.go` (syllago support), `docs/provider-sources/*.yaml` (native support + monitoring sources).
...
### factory-droid
All 6 content types. **Not yet implemented in syllago Go provider code** — planned provider. Hook schema matches Claude Code format exactly. Custom agents are called "Custom Droids".
...
### pi
**Not yet implemented in syllago Go provider code** — planned provider. Hooks are programmatic TypeScript extensions (not declarative JSON) — 27+ events. No MCP support (users implement via extensions). Commands are prompt templates (.pi/prompts/*.md).
...
### crush
**Not yet implemented in syllago Go provider code** — planned provider. Supports rules (AGENTS.md), skills (SKILL.md), MCP. No hooks (open issue charmbracelet/crush#2038). No agents, no commands.
...
| Skills       |      15        |      9       |         0            |        0          |      3      |          3           |
| Agents       |      15        |      5       |         3            |        2          |      3      |          2           |
| Commands     |      15        |      6       |         2            |        2          |      3      |          2           |
| MCP          |      15        |     10       |         0            |        0          |      3      |          2           |
| Hooks        |      15        |      8       |         0            |        0          |      3      |          4           |

**Rules** is the most universally supported content type — every implemented provider has it.  
**Hooks** is the most selective — only 8 of 12 implemented providers support lifecycle hooks.  
**Agents** has the most variation — true user-definable agent files in only 5 providers; 3 more use cross-provider AGENTS.md convention only.
