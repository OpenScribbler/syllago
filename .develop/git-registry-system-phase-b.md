...
- [x] Success criteria: `go test ./internal/registry/...` passes with `ok github.com/holdenhewett/nesco/cli/internal/registry`; all five `TestNameFromURL` cases pass; `make test` shows no failures in the registry package.
...
- [x] Missing context: The plan imports `"github.com/holdenhewett/nesco/cli/internal/config"` and `"github.com/holdenhewett/nesco/cli/internal/registry"` in `registries.go`. These imports bring in packages that are not currently imported in any `tui/` file. This creates a dependency from the `tui` package on the `registry` package. This is fine architecturally (it's a one-way dep), but an agent needs to know the module path: `github.com/holdenhewett/nesco/cli/internal/registry`. The plan shows this correctly.
