# 0011. per-provider anchor vs per-(provider, contentType) anchor
...
Chose **Per-provider anchor `<!-- capmon-check: <slug> -->` (new)** over **Per-(provider, contentType) anchor `<!-- capmon-check: <slug>/<contentType> -->` (`cli/internal/capmon/report.go:153`, `cli/internal/capmon/report.go:190`)**.

## Consequences

`FindOpenCapmonIssue` and `CreateCapmonChangeIssue` in report.go currently take `contentType string` as a parameter and embed it in the anchor. The new per-provider variants will omit `contentType` from the anchor and from their signatures. The existing `(provider, contentType)`-scoped functions remain unchanged because `onboard.go:81` calls `CreateCapmonChangeIssue` directly at the existing granularity and is explicitly out of scope for this change. Structure must introduce new `FindOpenCapmonProviderIssue` and `CreateCapmonProviderIssue` functions rather than modifying the existing ones.
