...
//
// codex, claude-code, amp, cline, copilot-cli, factory-droid are excluded from
// this batch. codex uses a multi-struct allow-list; the rest use landmark
...
// codex, claude-code, amp, cline, copilot-cli, factory-droid excluded — they
// use a different mechanism and have dedicated NoLandmarks tests in their own files.
func TestRecognizeGoStructBatch_EmptyFields(t *testing.T) {
	for _, slug := range []string{"kiro", "pi"} {
