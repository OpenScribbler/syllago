...
--keep: apply permanently; run "nesco loadout remove" to undo.`,
...
func runLoadoutApply(cmd *cobra.Command, args []string) error {
	...
	if len(args) == 1 {
		...
		if !found {
			output.PrintError(1, fmt.Sprintf("loadout %q not found", name), "Run 'nesco loadout list' to see available loadouts.")
	...
	if mode == "try" || mode == "keep" {
		...
		if snapErr == nil {
			...
			return fmt.Errorf("a loadout is already active. Run 'nesco loadout remove' first")
	...
	if mode == "try" {
		...
		fmt.Fprintln(output.Writer, "If auto-revert fails, run: nesco loadout remove")
	}
